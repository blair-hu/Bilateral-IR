function [bar_xtick,hb,bar_handles,he,errorbar_handles]=errorbar_groups(bar_input,errorbar_input,varargin)
%
            ...
            while ~isempty(varargin)
                ...
                switch varargin{1}
                    ...
                    case 'bar_colors'
                        ...
                        if size(bar_colors,1) > N_grps
                            disp('Setting individual bar colors');
                            individual_colors = 1;
                            temp_bar_colors = repmat([1 1 1],N_grps,1);
                        else
                            individual_colors = 0;
                            temp_bar_colors = bar_colors;
                        end
...
for grp=1:N_grps 
    bar_handles{grp} = bar( ...
        ...
        'FaceColor',temp_bar_colors(grp,:), ... % color parameter
        optional_bar_arguments{:}); % extra parameters
    hb(grp)=bar_handles{grp};
end

if individual_colors 
    % If re-coloring grouped bars, enter code here (use width of existing bar)
    bar_counter = 0;
    for grp_num=1:N_grps
        hold on;
        bar_handles_temp = bar_handles{grp_num};
        bar_inds = grp_num:N_bars:(N_grps*N_bars);
        for bar_num=1:N_bars
            bar(bar_handles_temp.XData(bar_num),bar_handles_temp.YData(bar_num),0.75,'FaceColor',bar_colors(bar_inds(bar_num),:));
        end
    end    
end
